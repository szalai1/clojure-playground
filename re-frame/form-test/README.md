...
* [`src/form_test/styles.cljs`](src/form_test/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/form_test/`](src/form_test/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/form_test/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `form-test` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/form_test/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns form-test.example
  (:require [form-test.config :as config])
